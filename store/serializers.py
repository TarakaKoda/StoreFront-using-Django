...
class AddCartItemSerializer(serializers.ModelSerializer):
    ...
    def validate_product_id(self, value):
        if not Product.objects.filter(pk=value).exists():
            raise serializers.ValidationError('product does not exist with this given ID')
        return value
...
class CartSerializer(serializers.ModelSerializer):
    ...
    def get_total_price(self, cart):
        total_price = [item.quantity * item.product.unit_price for item in cart.items.all()]
...
class CreateOrderSerializer(serializers.Serializer):
    ...
    def validate_cart_id(self, cart_id):
        if not Cart.objects.filter(pk=cart_id).exists():
            return serializers.ValidationError('No cart exists with the provided cart id')
